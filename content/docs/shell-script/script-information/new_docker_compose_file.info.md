...
`testing_docker-compose.yml`, and `my.cnf` files in the specified directory. If
it fails to create these files, it exits with an error message.

```bash
if ! touch "$dir_path/.env" "$dir_path/stack.env" "$dir_path/docker-compose.yml" "$dir_path/testing_docker-compose.yml" "$dir_path/my.cnf"; then
...
## Populating `docker-compose.yml` and `testing_docker-compose.yml` Files
