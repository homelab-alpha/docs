...
The script sets the filename, FQDN (Fully Qualified Domain Name), and IP
...
directory for the localhost certificate.
...
The script creates a CSR for the localhost certificate using the generated ECDSA
...
It creates a certificate chain bundle by concatenating the localhost certificate
...
The script includes steps to check the private key, CSR, localhost certificate,
and the chain bundle to verify their contents.
